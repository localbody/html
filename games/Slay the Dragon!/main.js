...
const insults = [
  'Nice try.',
  'Maybe aim for the white circle.',
  'That is it, huh?',
  'Try with your eyes open now.',
  'Perhaps we call it a draw?',
  'I am right in front of you.',
  'Ha. Ha. Ha.',
  'We are going to be here a while.',
  'Fear, I know thee not.',
  'Maybe next time.',
]
insults.sort((a, b) => Math.random() - 0.5)
var i_num = 0
  ...
  var run = setInterval(function () {
    if (xx < window.innerWidth && xx > 0 && yy < window.innerHeight && yy > 0) {
      ...
      var bull = actual.getBoundingClientRect(),
        bull_mid = document.elementFromPoint(
          bull.left + bull.width * 0.5,
          bull.top + bull.height * 0.5
        )

      if (
        bull_mid.classList.contains('heart') ||
        bull.top <= 0 ||
        ...
        if (bull_mid.classList.contains('heart')) {
          ...
          actual.style.setProperty(
            '--sword-shape',
            'polygon(50% 25%, 65% 25%, 65% 75%, 100% 75%, 100% 85%, 60% 85%, 60% 100%, 40% 100%, 40% 85%, 0% 85%, 0% 75%, 35% 75%, 35% 25%)'
          )
...
function dragging(e) {
  ...
  if (move && !run) {
    var y_diff = yy - pr_y,
      x_diff = pr_x - xx
    if (-y_diff * 0.08 > -15 && -y_diff * 0.08 < 0) {
      gd = -y_diff * 0.08
    }
    if (x_diff > 0) {
      power = x_diff * 0.05
