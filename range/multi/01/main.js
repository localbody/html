...
function setRightValue() {
  let min = parseInt($('#input-right').attr('min')),
    ...
    text = Math.max(
      parseInt($('#input-right').val()),
      parseInt($('#input-left').val()) + 1
    ),
    percent = ((text - min) / (max - min)) * 100

  $('#input-right').val(text)
  $('.price_value_max').text(
    text.toString().replace(/(\d)(?=(\d{3})+(\D|$))/g, '$1 ') + ' P'
  )
...
inputLeft.addEventListener('input', onInputLeft)
